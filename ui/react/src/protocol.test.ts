...
const mixed = parseJsonLines(
  [
    ...
    '{"type":"question_request","questions":[{"text":"Pick one","options":["A","B","C"],"allowMultiple":false}]}',
...
assert.equal(mixed.messages.length, 4);
assert.equal(mixed.messages[0]?.type, "response");
assert.equal(mixed.messages[1]?.type, "event_batch");
assert.equal(mixed.messages[2]?.type, "question_request");
assert.equal(mixed.messages[3]?.type, "status");
