...
from terminus.mission.models import (
...
from terminus.mission.store import MissionStore
from terminus.mission.tools import (
    MissionAskQuestion,
    normalize_scope,
    read_only_tools,
    scopes_overlap,
...
from terminus.prompts.system_prompt import get_agents_prompt
from terminus.tools import MissionDispatch
...
class MissionController:
    ...
    def _task_context(self, task: MissionTask) -> str:
        ...
        return (
            ...
            "recorded repository evidence, so never invent values. criteria_evidence must contain "
    ...
    def _usage_callback(self, task_id: str | None = None):
        def record(payload: dict[str, Any]) -> None:
            self.store.record_usage(self.mission_id, payload, task_id)
