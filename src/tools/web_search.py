...
class TavilyWebSearch:
    ...
    def search(
        ...
        try:
            response = requests.post(self.search_endpoint, json=payload, timeout=30)
    ...
    def get_answer(self, query: str) -> str:
        ...
        """
        result = self.search(
            query, max_results=5, include_answer=True, include_raw_content=False
        )
...
def web_search(
    ...
    return searcher.search(
        query, max_results=max_results, include_answer=include_answer
    )
...
class WebSearch(ToolSchema):
    ...
    Performs intelligent web searches and can provide AI-generated answers
    based on the search results. Useful for:
    - Research and fact-checking
    - Getting current information
    - Finding specific information online
    - Getting summaries of topics from multiple sources
    ...
    def description(self) -> str:
        ...
        - Current information (news, events, prices, releases)
        - Research and fact-checking
        - Finding specific details online
        - Summaries of topics from multiple sources
        - Data that may not be in the training data

        The tool returns search results with titles, URLs, content previews,
        relevance scores, and optionally an AI-generated answer synthesizing
        the results.

        Parameters:
        - query: The search query or question (required)
        - max_results: Number of results (1-10, default 5)
        - search_depth: "basic" (faster, less detail) or "advanced" (slower, more detail)
        """)
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "search_depth": {
                            "type": "string",
                            "description": "Search depth: 'basic' (faster, less detail) or 'advanced' (slower, more comprehensive)",
                            "enum": ["basic", "advanced"],
                            "default": "basic",
                        },
                    },
    ...
    def run(
        ...
        if not api_key:
            return (
                ...
                "Set it in your .env file or environment to enable web search."
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        result = tavily_web_search_tool(
            "What are the latest developments in AI in 2024?", max_results=3
        )
        print(result)
    except Exception as e:
